...
// orderId_infos[orderId].store must equal "temu." + MARKETPLACE.toLower().
...
void TestImporterFileTemuVatEu::test_marketplace_populatesOrderIdStore()
{
    ...
    // Return rows must also populate orderId_infos.
    ...
    // Order 5 — MARKETPLACE = "" (empty) → orderId_infos must NOT contain this id
    ...
    const auto &store = result.orderInfos->orderId_infos;

    // "FR" uppercase → "temu.fr"
    QVERIFY2(store.contains("PO-069-00000000000000070"),
             "orderId_infos must contain order 70");
    QCOMPARE(store.value("PO-069-00000000000000070").store, QString("temu.fr"));

    // "DE" uppercase → "temu.de"
    QVERIFY2(store.contains("PO-069-00000000000000071"),
             "orderId_infos must contain order 71");
    QCOMPARE(store.value("PO-069-00000000000000071").store, QString("temu.de"));

    // "it" already lowercase → "temu.it"
    QVERIFY2(store.contains("PO-069-00000000000000072"),
             "orderId_infos must contain order 72");
    QCOMPARE(store.value("PO-069-00000000000000072").store, QString("temu.it"));

    // return row with "ES" → "temu.es"
    QVERIFY2(store.contains("PO-069-00000000000000073"),
             "orderId_infos must contain return order 73");
    QCOMPARE(store.value("PO-069-00000000000000073").store, QString("temu.es"));

    // empty MARKETPLACE → no entry
    QVERIFY2(!store.contains("PO-069-00000000000000074"),
             "orderId_infos must NOT contain order 74 (empty marketplace)");
}

// ===========================================================================
// test_marketplace_columnAbsent
// When the MARKETPLACE column is absent the importer must not fail and
// orderId_infos must remain empty.
// ===========================================================================
void TestImporterFileTemuVatEu::test_marketplace_columnAbsent()
{
    ...
    QVERIFY2(result.errorReturned.isEmpty(), qPrintable(result.errorReturned));
    QCOMPARE(result.orderInfos->shipments.size(), 1);
    QVERIFY2(result.orderInfos->orderId_infos.isEmpty(),
             "orderId_infos must be empty when MARKETPLACE column is absent");
}
